"""Tests for `csklint.installation` module."""
...
from csklint.installation import NodeInstaller
...
class TestInstaller:
    ...
    def test_check_os_supported_macos(self, mocker: MockerFixture) -> None:
        mocker.patch("csklint.installation.platform.system", return_value="Darwin")
    ...
    def test_check_os_supported_unsupported_system(self, mocker: MockerFixture, tmp_path: Path) -> None:
        mocker.patch("csklint.installation.platform.system", return_value="Windows")
        assert Installer(tmp_path / "nonexistent").check_os_supported() is False
...
class TestNodeInstaller:
    ...
    def test_ensure_npm_present_does_nothing(self, mocker: MockerFixture) -> None:
        ...
        mocker.patch("csklint.installation.shutil.which", return_value="/usr/bin/npm")
        run_mock = mocker.patch("csklint.installation.subprocess.run")
    ...
    def test_ensure_npm_installs_nodejs_on_debian(self, mocker: MockerFixture) -> None:
        ...
        mocker.patch("csklint.installation.shutil.which", return_value=None)
        mocker.patch("csklint.installation.platform.system", return_value="Linux")
        run_mock = mocker.patch(
            "csklint.installation.subprocess.run",
    ...
    def test_ensure_npm_installs_nodejs_on_macos(self, mocker: MockerFixture) -> None:
        ...
        mocker.patch(
            "csklint.installation.shutil.which",
...
class TestSkillValidatorInstaller:
    ...
    def test_install_permission_error(self, mocker: MockerFixture, tmp_path: Path) -> None:
        ...
        mocker.patch("csklint.installation.shutil.copy2", side_effect=PermissionError)
