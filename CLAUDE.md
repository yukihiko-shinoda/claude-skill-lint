...
`claudeskilllint` (CLI name `csklint`) is a linter for Claude Code skills. Rather than reimplementing skill
...
- `claudeskilllint` → [claudeskilllint/cli.py](claudeskilllint/cli.py) `main()` — leftover cookiecutter scaffold
  placeholder, not the real interface.
- `csklint` → [claudeskilllint/cli.py](claudeskilllint/cli.py) `csklint` Click group — the actual CLI, with two
...
- **`tests/`** — the project's own test suite for this package's Python code (`installation.py`, `cli.py`, etc.). It
  mirrors `claudeskilllint/` 1:1 (e.g. `claudeskilllint/installation.py` → `tests/test_installation.py`) and is what
...
- **`claudeskilllint/_tests/`** — the linter's actual product: a pytest suite *shipped inside the wheel* (see its
  ...
  [claudeskilllint/_tests/tool_requirement.py](claudeskilllint/_tests/tool_requirement.py) when `skill-validator` or
  ...
  mocks. `tests/_tests/` mirrors `claudeskilllint/_tests/` the same way `tests/` mirrors `claudeskilllint/`
  (e.g. `tests/_tests/test_tool_requirement.py` tests `claudeskilllint/_tests/tool_requirement.py`).

### External tool installation ([claudeskilllint/installation.py](claudeskilllint/installation.py))
